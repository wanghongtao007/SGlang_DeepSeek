...
近年来，大语言模型（LLM）成为 AI 领域的核心技术，**DeepSeek** 作为国产领先的大模型之一，凭借其强大的推理能力和高效的架构，受到了广泛关注。而 **SGLang** 作为一款轻量级、高效的 LLM 应用开发框架，为开发者提供了便捷的 API 及推理部署能力。与此同时 **OpenShift** 及其 AI 组件 **OpenShift AI**（基于 Open Data Hub）提供了强大的 AI 工作负载管理能力，使得部署和扩展 LLM 变得更加高效。

本篇文章将带你手把手完成 **SGLang + DeepSeek** 大模型在 **OpenShift & OpenShift AI** 上的部署，让你快速搭建属于自己的 AI 交互服务。
...
SGLang 是一个专为 LLM（大语言模型）设计的高效开发框架，具备以下特点：
✅ 兼容 OpenAI API，易于迁移现有项目
✅ 高度可扩展，支持自定义模型和插件
✅ 低门槛快速集成，可用于 AI 应用开发
...
其中的**V3版本是通用型的大语言模型**，重点在于可扩展性和高效处理，目标是在多种自然语言处理任务中实现高效、灵活的应用。V3版本采用混合精度FP8训练，其训练分为高质量训练、扩展序列长度、进行SFT和知识蒸馏的后训练三个阶段。V3版本在数学、多语言任务以及编码任务中表现优秀，如在Cmath中得分90.7%，在Human Eval编码任务中通过率为65.2%。V3版本适用于大规模自然语言处理任务，如对话式AI、多语言翻译和内容生成等，能够为企业提供高效的AI解决方案，满足多领域的应用需求。
...
![image-20250224191257053](./assets/image-20250224191257053.png)

------

### 2.3 OpenShift 进行 LLM 高可用部署和推理

在当前的 AI 及大模型（LLM）应用浪潮中，企业越来越需要高效、可扩展、安全的基础设施来支持 AI 模型的开发、训练和推理。传统的物理机或者虚拟化基础设施在运维和自动化等方面越来越不如容器平台给客户带来更多便捷性和增值服务。而 OpenShift 作为基于 Kubernetes（K8s） 的企业级容器平台，提供了一整套 AI/ML 解决方案，特别适合 大语言模型（LLM） 及 AI 负载的部署与管理。

Operator提供了一种打包、部署和管理其Kubernetes应用程序的方法，简化了客户管理的应用程序的安装和持续维护。例如NVIDIA GPU Operator使用Kubernetes中的Operator框架来自动管理在Kubernetes集群上配置GPU所需的所有NVIDIA软件组件。NVIDIA GPU Operator使向OpenShift添加GPU资源成为集群运维管理员的一个优选。这使得客户在部署诸如大模型类的模块时，无需耗费精力安装、运维后台显卡的各种复制驱动及版本配置。

如下图：

![image-20250226121838828](./assets/image-20250226121838828.png)

除此之外，OpenShift部署大模型有以下优势：
...
#### (2) 为什么 OpenShift AI 适合 LLM（大模型）部署？

在 OpenShift AI 上，可以**高效地部署和管理 LLM（如 DeepSeek、Llama、ChatGLM）**，具有以下优势：
...
一般情况下，模型参数所耗费的显存和内存的配置如下表，我们在使用大模型之前，可以先掂量一下手里的存货再继续：
...
你可以通过 OpenShift Web 控制台访问 OpenShift AI Dashboard 并创建 Notebook 或 AI 服务。
